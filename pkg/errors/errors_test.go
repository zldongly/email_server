...
func TestWithCode(t *testing.T) {
	tests := []struct {
		...
		{io.EOF, 500, "server err",
			"code = 500, message = server err, cause = EOF"},
	}

	for _, tt := range tests {
		got := WithCode(tt.err, tt.code, tt.message).Error()
...
func TestCode(t *testing.T) {
	tests := []struct{
		...
		{io.EOF, 0},
		{WithCode(io.EOF, 500, ""), 500},
		{WithMessage(WithCode(io.EOF, 500, ""), "test message"), 500},
	}
